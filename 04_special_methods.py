...
class Backpack:
    ...
    def __bool__(self):
        return self.content != [] # проверяет если объект не пустой то вернет True, а если пустой то вернет False.

    def __len__(self):
        return len(self.content)  # проверяет какое количество находится на данный момент


my_backpack = Backpack()
my_backpack.add(item='ноутбук')
print(bool(my_backpack), len(my_backpack))  # пока что логику пишем после объявления объекта
if my_backpack:
